...
#ifndef _WIN32
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_opencv_image_validation) {
  // loading template orginal img
  cv::Mat input_image =
      cv::imread(ppc::util::GetAbsolutePath("omp/varfolomeev_g_histogram_linear_stretching/data/cobble_orig.jpg"),
                 cv::IMREAD_GRAYSCALE);
  ASSERT_FALSE(input_image.empty());

  // loading template modified img
  cv::Mat reference_image =
      cv::imread(ppc::util::GetAbsolutePath("omp/varfolomeev_g_histogram_linear_stretching/data/cobble_mod.jpg"),
                 cv::IMREAD_GRAYSCALE);
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_opencv_manual_64x64) {
  // loading template orginal img
  cv::Mat input_image =
      cv::imread(ppc::util::GetAbsolutePath("omp/varfolomeev_g_histogram_linear_stretching/data/cobble_orig.jpg"),
                 cv::IMREAD_GRAYSCALE);
  ...
  task_data_omp->inputs.emplace_back(reinterpret_cast<uint8_t *>(input_vector.data()));
  task_data_omp->inputs_count.emplace_back(input_vector.size());
  task_data_omp->outputs.emplace_back(reinterpret_cast<uint8_t *>(output_vector.data()));
  task_data_omp->outputs_count.emplace_back(output_vector.size());

  varfolomeev_g_histogram_linear_stretching_omp::TestTaskSequential test_task_ompuential(task_data_omp);
  EXPECT_TRUE(test_task_ompuential.ValidationImpl());
  test_task_ompuential.PreProcessingImpl();
  test_task_ompuential.RunImpl();
  test_task_ompuential.PostProcessingImpl();
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_manual_9) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_manual_10) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_manual_25) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_stretched) {
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_boundary_values) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_flat) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_single_non_flat_pixel_2) {
  // Create data
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_10k_generated) {
  // Create data
  const int sz = 10000;
...
TEST(varfolomeev_g_histogram_linear_stretching_omp, test_empty) {
  // Create data
  ...
  // Create Task
  varfolomeev_g_histogram_linear_stretching_omp::TestTaskSequential test_task_ompuential(task_data_omp);
  ASSERT_EQ(test_task_ompuential.Validation(), false);
}

TEST(varfolomeev_g_histogram_linear_stretching_omp, test_in_out_NE) {
  // Create data
