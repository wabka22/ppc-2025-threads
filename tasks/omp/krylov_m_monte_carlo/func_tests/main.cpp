...
class krylov_m_monte_carlo_test_omp  // NOLINT(readability-identifier-naming)
...
#define EXEC_TASK(Task, params, out)         \
  ...
  static void RunTest(IntegrationParams&& params, double ref) {
    double out{};
    EXEC_TASK(TaskOpenMP, params, out);
...
TEST_F(krylov_m_monte_carlo_test_omp, sample_5d) {
  RunUndeterminedTest(GenerateSampleParams(
      5, [](const Point& x) { return std::pow(std::numbers::e, -std::reduce(x.begin(), x.end(), 0.)); }, 50'000));
}

TEST_F(krylov_m_monte_carlo_test_omp, validation_failure) {
  IntegrationParams params{.func = [](const Point&) { return 0.; }, .bounds = {{1, 0}}, .iterations = 100'000};
  double stub{};

  TaskOpenMP task(params.CreateTaskData(stub));
  EXPECT_FALSE(task.Validation());
}

TEST_P(krylov_m_monte_carlo_test_omp, determined) {
  auto [params, ref] = GetParam();
  RunTest(std::move(params), ref);
}

// clang-format off
INSTANTIATE_TEST_SUITE_P(krylov_m_monte_carlo_test_omp, krylov_m_monte_carlo_test_omp, ::testing::Values( // NOLINT(misc-use-anonymous-namespace)
    DeterminedTestCase{
        {
            .func = [](const Point& x) {
                return 0.;
