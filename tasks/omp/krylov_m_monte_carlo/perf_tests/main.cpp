...
class krylov_m_monte_carlo_test_omp : public ::testing::Test {  // NOLINT(readability-identifier-naming)
 protected:
  static void RunPerfTest(const std::function<void(ppc::core::Perf &, const std::shared_ptr<ppc::core::PerfAttr> &,
    ...
    auto task = std::make_shared<TaskOpenMP>(params.CreateTaskData(out));
...
TEST_F(krylov_m_monte_carlo_test_omp, test_pipeline_run) {
  RunPerfTest([](auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
    perf_analyzer.PipelineRun(perf_attr, perf_results);
  });
}

TEST_F(krylov_m_monte_carlo_test_omp, test_task_run) {
  RunPerfTest([](auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
