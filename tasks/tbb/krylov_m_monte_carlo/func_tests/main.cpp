...
class krylov_m_monte_carlo_test_tbb  // NOLINT(readability-identifier-naming)
...
TEST_F(krylov_m_monte_carlo_test_tbb, sample_5d) {
  RunUndeterminedTest(GenerateSampleParams(
      5, [](const Point& x) { return std::pow(std::numbers::e, -std::reduce(x.begin(), x.end(), 0.)); }, 50'000));
}

TEST_F(krylov_m_monte_carlo_test_tbb, validation_failure) {
  IntegrationParams params{.func = [](const Point&) { return 0.; }, .bounds = {{1, 0}}, .iterations = 100'000};
  double stub{};

  TaskTBB task(params.CreateTaskData(stub));
  EXPECT_FALSE(task.Validation());
}

TEST_P(krylov_m_monte_carlo_test_tbb, determined) {
  auto [params, ref] = GetParam();
  RunTest(std::move(params), ref);
}

// clang-format off
INSTANTIATE_TEST_SUITE_P(krylov_m_monte_carlo_test_tbb, krylov_m_monte_carlo_test_tbb, ::testing::Values( // NOLINT(misc-use-anonymous-namespace)
    DeterminedTestCase{
        {
            .func = [](const Point& x) {
                return 0.;
