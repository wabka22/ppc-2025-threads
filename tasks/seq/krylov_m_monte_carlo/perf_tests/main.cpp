...
class krylov_m_monte_carlo_test_seq : public ::testing::Test {  // NOLINT(readability-identifier-naming)
 protected:
  static void RunPerfTest(const std::function<void(ppc::core::Perf &, const std::shared_ptr<ppc::core::PerfAttr> &,
    ...
    auto task = std::make_shared<TaskSequential>(params.CreateTaskData(out));
...
TEST_F(krylov_m_monte_carlo_test_seq, test_pipeline_run) {
  RunPerfTest([](auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
    perf_analyzer.PipelineRun(perf_attr, perf_results);
  });
}

TEST_F(krylov_m_monte_carlo_test_seq, test_task_run) {
  RunPerfTest([](auto &perf_analyzer, const auto &perf_attr, const auto &perf_results) {
